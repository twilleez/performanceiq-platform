...
- Authenticated production shell regression resolved by styling the actual `#piq-topbar`, `#piq-layout`, `#piq-sidebar`, `.sidebar-link`, and `.main-content` contract generated by `js/app.js`.
- A Playwright shell regression test now verifies desktop grid layout, sidebar/content sizing, topbar sizing, and non-black navigation text.
- Shell-fix Browser Core Journeys run `33210051614` PASS.
- Shell-fix Production Smoke Checks run `33210051589` PASS.
- Shell-fix GitHub Pages deployment run `33210051598` PASS.
...
### Active phase: Launch Operations
- Maintain automated browser, smoke, and deploy gates on `main`.
- Treat user-visible regressions as release blockers and add a regression test with every confirmed fix.
- Use the retained QA role accounts/relationships for authorization regression testing.
- Keep paid pricing/billing out of production until Stripe purchase/provisioning testing exists.
- Track the remaining Supabase leaked-password configuration separately from application code.
