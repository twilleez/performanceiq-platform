/* ================================================================
   PerformanceIQ — core.js  v5.1  Production-Ready
   Fixed: localStorage persistence, switchView guard, detail panel,
   duplicate listeners, null-safe wellness colors, analytics indices,
   import handler, settings case-mismatch, keyboard nav, chart scaling.
   ================================================================ */
'use strict';
...
function renderDashboard() {
  ...
  el('statMonitorSub').textContent = monitor > 0 ? `↑ Check load today` : 'All clear';

  const badge = el('riskBadge');
  const flags = risk + monitor;
  badge.textContent   = flags;
  badge.style.display = flags > 0 ? 'flex' : 'none';

  el('chipOnlineText').textContent = `${ATHLETES.length - 1} online`;
  el('chipFlags').style.display    = risk > 0 ? 'inline-flex' : 'none';
  el('chipFlagsText').textContent  = `${risk} flag${risk !== 1 ? 's' : ''}`;
  el('chipGame').style.display     = 'inline-flex';
  el('chipGameText2').textContent  = `Game in ${EVENTS[0].days}d`;
...
function openAthleteDetail(a) {
  // Navigate to athletes view without the early-return guard resetting detail
  ...
  el('athleteDetail').style.display  = 'flex';
  ...
  el('detailHero').innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;align-items:center;gap:6px;flex-shrink:0">
      <div style="font-family:var(--font-display);font-size:44px;font-weight:800;line-height:1;color:${color}">${a.score||'—'}</div>
      <div class="score-tier ${t.cls}" style="font-size:11px">${t.label}</div>
    </div>`;
  ...
  // Pillars
  el('detailPillars').innerHTML = getPillars(a).map(p => `
    <div class="pillar">
  ...
  el('detailWellness').innerHTML = wItems.map(w => `
  ...
  // Load bar
  el('detailLoad').innerHTML = a.acr != null
  ...
  // PRs
  const prTbody = el('detailPRs').querySelector('tbody');
...
function renderGeneratedSession() {
  ...
  // Listeners on fresh DOM — no duplicates
...
function renderAnalytics() {
  ...
  // 7-day load chart — properly scaled to max AU value
  const maxAU = Math.max(...WEEK_LOAD.map(d => d.au));
  el('loadChart').innerHTML = WEEK_LOAD.map(d => {
  ...
  el('scoreDistChart').innerHTML = ranges.map(r =>
  ...
  // Trend table — correct weekHistory indices [0]=W-4 [2]=W-3 [4]=W-2 [6]=now
  el('analyticsBody').innerHTML = ATHLETES.filter(a => a.score > 0).map(a => {
...
function switchView(viewId) {
  ...
  // Reset athletes sub-state when leaving
...
function handleSearch(value) {
  ...
  el('athleteSearch').value      = value;
  el('athleteFilterInput').value = value;
}
...
el('settingTeamName').value = STATE.teamName;
Array.from(el('settingSport').options).forEach(o => {
  o.selected = o.value.toLowerCase() === STATE.sport;
});

el('btnSaveSettings').addEventListener('click', () => {
  STATE.teamName = el('settingTeamName').value.trim() || STATE.teamName;
  STATE.season   = el('settingSeason').value;
  STATE.sport    = el('settingSport').value.toLowerCase(); // fix: normalize to lowercase
  el('userRole').textContent = `Head Coach · ${cap(STATE.sport)}`;
  saveState();
  renderDashboard();
...
function goObStep(n) {
  ...
  el('obProgress').style.width = Math.round(n / 3 * 100) + '%';
}
...
el('obNext2').addEventListener('click', () => {
  el('obFirstSession').innerHTML = buildWorkoutCardHTML(generateSession(obSport,'practice',60,'moderate',[]), false);
  goObStep(3);
});
...
el('obFinish').addEventListener('click', () => {
  ...
  el('userName').textContent = LABELS[obRole] || 'Coach Davis';
  el('userRole').textContent = `${LABELS[obRole]||'Head Coach'} · ${cap(obSport)}`;
...
function init() {
  ...
  if (!seen) el('onboardingModal').style.display = 'flex';
  el('userRole').textContent = `Head Coach · ${cap(STATE.sport)}`;
  renderDashboard();
}
