/* ================================================================
   PerformanceIQ — core.js  v5.2.1  (Elite UI Pass — FIXED)
   Fixes:
   - Removed duplicate theme + sport-theme functions (no redeclare errors)
...
// Some parts of older code used persistState()
...
function toast(msg, ms = 2800) {
  const c = el('toastContainer') || el('toastcontainer');
  const host = c || (() => {
...
function animateNumber(node, from, to, ms = 650) {
  ...
  function tick(now) {
    ...
    const val = Math.round(a + (b - a) * eased);
...
function ensureTabIndicator() {
  ...
  if (!navHost.style.position) navHost.style.position = 'relative';
...
function renderDashboard(){
  ...
  if (el('statAvg')) {
    ...
    if (avg) tweenNumber(el('statAvg'), avg, 520);
  }
  ...
  if (el('chipGameText2')) el('chipGameText2').textContent = `Game in ${EVENTS[0]?.days ?? '—'}d`;
  ...
  if (el('pillGameText')) el('pillGameText').textContent = `Game in ${EVENTS[0]?.days ?? '—'} days`;
...
function openAthleteDetail(a){
  ...
  const heroScore = el('detailHeroScore');
...
function generateSession(sport, type, duration, intensity, injuries){
  ...
  // Optional: add exercises into blocks (kept lightweight)
