    ...
    <div className="piq-page">
      ...
      {subPage === "analytics" && (
        <div>
          ...
          {trend.length > 0 && <div style={{ marginBottom: 20 }}><ProgressTrend data={trend} label="30-Day PIQ Score" /></div>}
          {milestones.length > 0 && <MilestoneGrid milestones={milestones} />}
          {trend.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 20px", background: "var(--bg-card)", border: "1px solid var(--border-default)", borderRadius: 12 }}>
              ...
              <div style={{ fontSize: 14, color: "var(--text-muted)", lineHeight: 1.65 }}>Analytics will appear after you log a few sessions.</div>
            </div>
          )}
        </div>
      )}
      ...
      {subPage === "team" && (
        <div>
          ...
          <div style={{ textAlign: "center", padding: "40px 20px", background: "var(--bg-card)", border: "1px solid var(--border-default)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 14, color: "var(--text-muted)", lineHeight: 1.65 }}>Invite athletes to your team to see their PIQ Scores, compliance rates, and activity streaks here.</p>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          {user?.role === "coach" || user?.role === "admin" ? (
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-default)", borderRadius: 12, padding: "20px" }}>
              <div style={{ fontSize: 14, color: "var(--text-muted)", marginBottom: 16, lineHeight: 1.65 }}>Generate a branded PDF progress report for any athlete. Share via link — no login required for parents.</div>
              <button style={{ height: 44, padding: "0 24px", borderRadius: 10, border: "none", background: "var(--theme-primary)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }} onClick={() => analytics.track("report_generate_clicked")}>
                Generate Report →
              </button>
            </div>
          ) : (
            <div style={{ textAlign: "center", padding: "40px 20px", background: "var(--bg-card)", border: "1px solid var(--border-default)", borderRadius: 12 }}>
              <div style={{ fontSize: 36, marginBottom: 12 }}>📊</div>
              <div style={{ fontSize: 14, color: "var(--text-muted)", lineHeight: 1.65 }}>Your coach's reports will appear here once they generate one for your athlete.</div>
            </div>
          )}
        </div>
      )}
