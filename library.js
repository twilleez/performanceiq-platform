...
export function renderCoachSettings() {
  ...
  const initials = (user?.name||'C').split(' ').map(w=>w[0]).slice(0,2).join('');
  return `
  ...
  <main class="page-main">
    ...
    <div class="panels-2">
      <div>
        <div class="panel" style="margin-bottom:16px">
          ...
          <div style="display:flex;align-items:center;gap:14px;margin:14px 0">
            ...
            <div><div style="font-weight:700;font-size:15px;color:var(--text-primary)">${user?.name||'Coach'}</div><div style="font-size:12.5px;color:var(--text-muted)">${user?.email||''} · Coach</div></div>
          </div>
      ...
      <div class="panel">
        ...
        ${[['Athlete readiness alerts',true],['Caution flag notifications',true],['Weekly summary digest',true],['New message alerts',true],['Program update reminders',false]].map(([l,on])=>`
