/**
 * Solo Library View — exercise library for solo athletes
 */
...
export function renderSoloLibrary() {
  ...
  const sportExercises = EXERCISES.filter(e => e.sports.includes(sport));
  const featuredExercises = (sportExercises.length >= 6 ? sportExercises : EXERCISES).slice(0,8).map(e => `
  ...
  const soloTemplates = TRAINING_TEMPLATES.filter(t => t.sport === sport || t.level === 'All Levels').slice(0,3);
  const templateCards = (soloTemplates.length > 0 ? soloTemplates : TRAINING_TEMPLATES.slice(0,3)).map(t => `
  <div class="panel" style="padding:16px">
    ...
    <div style="font-size:12px;color:var(--text-primary);font-weight:600;margin-bottom:8px">Focus: ${t.focus}</div>
