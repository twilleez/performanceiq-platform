    ...
    <div class="panels-2">
      <div>
        <div class="panel" style="margin-bottom:16px">
          ...
          <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
            <div style="width:56px;height:56px;border-radius:50%;background:var(--piq-green);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:#0d1b3e">
              ${(user?.name||'S').split(' ').map(w=>w[0]).slice(0,2).join('')}
            </div>
            <div>
              <div style="font-weight:700;font-size:15px;color:var(--text-primary)">${user?.name||'Solo Athlete'}</div>
              <div style="font-size:12.5px;color:var(--text-muted)">${user?.email||''} · Solo</div>
            </div>
          </div>
        ...
        <div class="panel">
          ...
          <div style="font-size:13px;color:var(--text-muted);margin-bottom:10px;margin-top:8px">Free Plan · Upgrade for advanced features</div>
          <button class="btn-primary" style="font-size:13px;padding:10px 18px;width:100%" data-route="solo/subscription">Manage Subscription</button>
        </div>
      </div>
      <div class="panel">
        ...
        ${[
          ...
          ['Readiness alerts', true],
          ['Streak milestones', true],
          ['Progress summaries', false],
