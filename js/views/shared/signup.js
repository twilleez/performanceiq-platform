...
<div class="auth-card" style="margin-top:0" aria-labelledby="su-title">
  ...
  <form id="su-form" novalidate>
    <div class="input-group">
      <div class="input-wrap">
        <label class="sr-only" for="su-name">Full name</label>
        <input type="text" id="su-name" placeholder="Full name" autocomplete="name" required maxlength="100">
      </div>
      <div class="input-wrap">
        <label class="sr-only" for="su-email">Email address</label>
        <input type="email" id="su-email" placeholder="Email address" autocomplete="email" inputmode="email" required maxlength="254">
      </div>
      <div class="input-wrap">
        <label class="sr-only" for="su-pass">Create password</label>
        <input type="password" id="su-pass" placeholder="Create password" autocomplete="new-password" required minlength="8" aria-describedby="su-pass-help">
        <span id="su-pass-help" class="auth-help">Use at least 8 characters.</span>
      </div>
    </div>
  ...
  <div class="auth-foot" style="margin-top:14px">
    Already have an account? <button type="button" id="su-signin-link" class="auth-inline-link">Sign in</button>
  </div>
</div>`;
}
...
document.addEventListener('piq:authRendered', () => {
  ...
  document.querySelectorAll('#su-role-tabs .role-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      document.querySelectorAll('#su-role-tabs .role-tab').forEach(t => {
  ...
  document.getElementById('su-form')?.addEventListener('submit', async event => {
    ...
    if (!nameEl?.checkValidity() || !emailEl?.checkValidity() || !passEl?.checkValidity()) {
      if (msgEl) {
        msgEl.textContent = passEl && !passEl.checkValidity()
