 ...
 * Premium redesign using app token system.
 * All navigation wired to router.js navigate() and auth.js signIn().
 */
...
<div class="wlc-root">
  ...
  <footer class="wlc-footer">
    ...
    <div class="wlc-footer-right">
      <button class="wlc-footer-link">Privacy</button>
      <button class="wlc-footer-link">Terms</button>
      <button class="wlc-footer-link">Contact</button>
    </div>
  </footer>
...
.wlc-nav{position:relative;z-index:10;display:flex;align-items:center;justify-content:space-between;padding:0 24px;height:60px;border-bottom:1px solid var(--piq-border);background:rgba(1,13,20,0.85);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);flex-shrink:0}
...
.wlc-hero{position:relative;z-index:1;padding:64px 24px 52px;max-width:820px}
.wlc-eyebrow{display:inline-flex;align-items:center;gap:12px;font-family:var(--font-num);font-size:11px;font-weight:500;letter-spacing:4px;text-transform:uppercase;color:var(--piq-green);margin-bottom:22px;animation:wlc-up 0.5s ease both}
.wlc-eyebrow-line{display:block;width:22px;height:1px;background:var(--piq-green)}
.wlc-h1{font-family:var(--font-display);font-size:clamp(34px,6.5vw,62px);font-weight:700;line-height:1.0;letter-spacing:-0.5px;color:#fff;margin-bottom:20px;animation:wlc-up 0.5s 0.08s ease both}
.wlc-accent{color:var(--piq-green)}
.wlc-sub{font-size:15px;font-weight:300;color:rgba(208,238,244,0.6);line-height:1.75;max-width:500px;margin-bottom:36px;animation:wlc-up 0.5s 0.15s ease both}
.wlc-actions{display:flex;align-items:center;gap:16px;flex-wrap:wrap;margin-bottom:44px;animation:wlc-up 0.5s 0.22s ease both}
...
.wlc-trust{display:flex;align-items:center;gap:22px;flex-wrap:wrap;padding-top:36px;border-top:1px solid var(--piq-border);animation:wlc-up 0.5s 0.28s ease both}
...
.wlc-stat{flex:1;padding:22px 12px;text-align:center;border-right:1px solid var(--piq-border);transition:background 0.2s}
.wlc-stat:last-child{border-right:none}
.wlc-stat:hover{background:rgba(111,217,79,0.04)}
.wlc-stat-n{font-family:var(--font-display);font-size:26px;font-weight:700;color:var(--piq-green);letter-spacing:-0.5px;line-height:1;margin-bottom:4px}
.wlc-stat-l{font-family:var(--font-num);font-size:9.5px;font-weight:500;color:var(--piq-muted);letter-spacing:1.8px;text-transform:uppercase}
.wlc-roles-section{position:relative;z-index:1;padding:40px 24px 64px;flex:1}
.wlc-section-label{font-family:var(--font-num);font-size:10px;font-weight:500;letter-spacing:3.5px;text-transform:uppercase;color:rgba(74,120,136,0.6);text-align:center;margin-bottom:24px}
.wlc-roles{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:var(--piq-border);border:1px solid var(--piq-border);border-radius:8px;overflow:hidden;max-width:960px;margin:0 auto}
.wlc-role-card{background:var(--piq-s1);padding:26px 20px 22px;position:relative;cursor:pointer;text-align:left;border:none;color:inherit;font-family:var(--font-body);transition:background 0.25s;display:flex;flex-direction:column}
.wlc-role-card:hover{background:var(--piq-s2)}
.wlc-role-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--piq-green),rgba(111,217,79,0.15));transform:scaleX(0);transform-origin:left;transition:transform 0.3s cubic-bezier(0.4,0,0.2,1)}
.wlc-role-card:hover::before{transform:scaleX(1)}
.wlc-role-card:disabled{opacity:0.55;cursor:not-allowed;pointer-events:none}
.wlc-role-icon{width:42px;height:42px;border-radius:9px;background:var(--piq-green-glow);border:1px solid var(--piq-green-border);display:flex;align-items:center;justify-content:center;margin-bottom:16px;flex-shrink:0;transition:background 0.25s,border-color 0.25s}
.wlc-role-card:hover .wlc-role-icon{background:rgba(111,217,79,0.18);border-color:rgba(111,217,79,0.4)}
.wlc-role-name{font-family:var(--font-display);font-size:17px;font-weight:600;color:#fff;letter-spacing:0.4px;margin-bottom:4px}
.wlc-role-desc{font-size:11px;font-weight:300;color:var(--piq-muted);line-height:1.6;margin-bottom:14px}
.wlc-role-list{list-style:none;display:flex;flex-direction:column;gap:7px;flex:1;margin:0;padding:0}
...
.wlc-role-cta{font-family:var(--font-num);font-size:10px;font-weight:500;letter-spacing:1.5px;text-transform:uppercase;color:var(--piq-green);margin-top:20px;opacity:0.6;transition:opacity 0.2s}
.wlc-role-card:hover .wlc-role-cta{opacity:1}
.wlc-footer{position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-top:1px solid var(--piq-border);flex-wrap:wrap;gap:12px;flex-shrink:0}
.wlc-footer-left{font-size:11px;color:rgba(74,120,136,0.5)}
.wlc-footer-right{display:flex;gap:20px}
.wlc-footer-link{background:none;border:none;cursor:pointer;font-family:var(--font-body);font-size:11px;color:rgba(74,120,136,0.5);padding:0;transition:color 0.2s}
.wlc-footer-link:hover{color:var(--piq-muted)}
@keyframes wlc-up{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
@media(max-width:860px){.wlc-roles{grid-template-columns:repeat(2,1fr)}}
@media(max-width:540px){.wlc-nav{padding:0 16px}.wlc-logo-sub{display:none}.wlc-hero{padding:48px 16px 40px}.wlc-roles-section{padding:32px 16px 48px}.wlc-roles{grid-template-columns:1fr}.wlc-stats{flex-wrap:wrap}.wlc-stat{min-width:50%;border-bottom:1px solid var(--piq-border)}.wlc-footer{padding:16px}.wlc-trust{gap:14px}.wlc-actions{flex-direction:column;align-items:flex-start;gap:12px}}
</style>`;
}

// ── Event wiring — runs after HTML is injected ─────────────────────────────
document.addEventListener('piq:authRendered', () => {

  // "Get started" CTA → signup flow
  document.getElementById('wlc-get-started')
    ?.addEventListener('click', () => navigate(ROUTES.SIGN_UP));

  // Hero ghost "Sign in" link
  ...
  // Demo role cards — sign in as demo user and navigate to their home
  document.querySelectorAll('.wlc-role-card[data-demo]').forEach(card => {
    card.addEventListener('click', async () => {
      ...
      try {
        ...
        if (ctaEl) ctaEl.textContent = origText || '⚠ Error';
        card.disabled = false;
      }
    });
  });
