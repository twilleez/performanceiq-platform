// js/services/workoutService.js — PerformanceIQ production workout service
import { supabase } from '../core/supabase.js'
import { getCurrentUser, getSession } from '../core/auth.js'
...
function isDemo() { return !!getSession()?.isDemo }
...
export async function getExerciseCategoryCounts() {
  ...
  return Object.entries(byCategory).map(([category, exs]) => ({ category, count: exs.length }))
}
...
/** Linked/visible Player profiles. RLS is the authority for real Coach accounts. */
export async function getAssignableAthletes() {
  ...
  const { data, error } = await supabase
    .from('profiles')
    .select('id,name,email,sport,position,role')
    .eq('role', 'player')
    .order('name')
...
/** Create a real Coach → Player workout assignment in public.workouts. */
export async function assignWorkoutToAthlete({
  athleteId, title, sport, dayType = 'Strength', scheduledDate,
  exercises = [], notes = '', recoveryCue = '', templateId = null
}) {
  const coach = currentUser()
  if (!coach?.id) throw new Error('Coach authentication is required.')
  if (!athleteId) throw new Error('Select an athlete.')
  if (!title?.trim()) throw new Error('Workout name is required.')
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      title: title.trim(),
      sport: sport || null,
      day_type: String(dayType || 'Strength').toLowerCase(),
...
export async function getWorkouts({ from, to, status, limit = 50 } = {}) {
  const user = currentUser()
  if (!user || isDemo()) return _getLocalWorkouts()
  let q = supabase
    .from('workouts')
    .select('*')
...
export async function getTodayWorkout() {
  ...
  return rows.find(w => w.status !== 'completed' && !w.completed_at) || rows[0] || null
}
...
export async function createWorkout({ title, sport, dayType = 'strength', scheduledDate, exercises = [], notes = '', recoveryCue = '', templateId = null, assignedBy = null }) {
  const user = currentUser()
  if (!user || isDemo()) {
  ...
  const { data, error } = await supabase.from('workouts').insert({
    athlete_id: user.id, title, sport: sport ?? null, day_type: dayType,
    scheduled_date: scheduledDate ?? _today(), exercises, notes,
    recovery_cue: recoveryCue, template_id: templateId,
    assigned_by: assignedBy, status: 'planned'
  }).select().single()
  if (error) throw error
...
export async function updateWorkout(id, updates) {
  ...
  const { data, error } = await supabase.from('workouts')
    ...
    .eq('id', id).eq('athlete_id', user.id).select().single()
  if (error) throw error
  _clearCache(CACHE_KEY_WO)
  return data
}
...
export async function deleteWorkout(id) {
  const user = currentUser()
  if (!user) throw new Error('Not authenticated')
  const { error } = await supabase.from('workouts').delete().eq('id', id).eq('athlete_id', user.id)
  if (error) throw error
  _clearCache(CACHE_KEY_WO)
}
...
export async function completeWorkout(id, { exerciseLogs, durationMin, rpeActual, notes }) {
  ...
  const payload = {
    ...
    exercise_logs: exerciseLogs ?? [], duration_min: durationMin ?? null,
    rpe_actual: rpeActual ?? null, notes: notes ?? null,
  ...
  const { data, error } = await supabase.from('workouts').update(payload)
    .eq('id', id).eq('athlete_id', user.id).select().single()
...
export async function getStreak() {
  ...
  const { data } = await supabase.from('workouts').select('scheduled_date')
    .eq('athlete_id', user.id).eq('status', 'completed')
    .order('scheduled_date', { ascending:false }).limit(60)
  ...
  while (true) {
    ...
    if (dateSet.has(ds)) { streak++; check.setDate(check.getDate()-1) } else break
  }
  return streak
}

function _getLocalWorkouts() { try { return JSON.parse(localStorage.getItem(CACHE_KEY_WO) || '[]') } catch { return [] } }
function _updateLocalWorkout(id, updates) {
  const rows = _getLocalWorkouts(); const i = rows.findIndex(r => r.id === id)
  if (i >= 0) rows[i] = { ...rows[i], ...updates }
  localStorage.setItem(CACHE_KEY_WO, JSON.stringify(rows)); return rows[i]
}
