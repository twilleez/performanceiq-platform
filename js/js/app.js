...
async function init() {
  ...
  // Branded splash — animated progress bar, minimum 1 second
...
async function renderRoute(route) {
  ...
  if (!entry) {
    // Unknown route → go home for authenticated users, welcome otherwise
    const fallback = VIEW_MAP[
      isAuthenticated()
        ? (ROLE_HOME[getCurrentRole()] || ROUTES.WELCOME)
        : ROUTES.WELCOME
    ];
